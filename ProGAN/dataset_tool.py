...
def create_celeba(tfrecord_dir, celeba_dir, cx=89, cy=121, shuffle=1, num_images=0, num_shifts=0):
    print('Loading CelebA from "%s"' % celeba_dir)
    glob_pattern = os.path.join(celeba_dir, '*.png')
    image_filenames = sorted(glob.glob(glob_pattern))
    
    if num_images == 0:
        image_filenames_new = list(image_filenames)
    elif num_images != 0 and num_shifts == 0:
        image_filenames_new = image_filenames[:num_images]
    elif num_images != 0 and num_shifts != 0:
        image_filenames_new = image_filenames[:num_images-num_shifts] + image_filenames[-num_shifts:]
    
    with TFRecordExporter(tfrecord_dir, len(image_filenames_new)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames_new))
        for idx in range(order.size):
            img = np.asarray(PIL.Image.open(image_filenames_new[order[idx]]))
...
def create_from_images(tfrecord_dir, image_dir, shuffle=1, num_images=0, num_shifts=0):
    ...
    img = np.asarray(PIL.Image.open(image_filenames_new[0]))
